...
void *Led1Hz(void *userdata);   //Lチカのスレッド関数
void *Orgel(void *userdata);    //オルゴールのスレッド関数
...
int main (void){
    ...
    int i;
    int qN = 1000;                      //音の長さ(ms)
    int ledRun = LG_LOW;    //Lチカスレッドのステータス、LG_LOW：停止状態、LG_HIGH：動作状態
    int orgelRun = LG_LOW;  //オルゴールスレッドのステータス、LG_LOW：停止状態、LG_HIGH：動作状態
    ...
    while(1){
        ...
        switch(i){
            case 1:
                if(ledRun == LG_LOW){   //停止状態のとき、Lチカスレッドスタート
                    ledRun=LG_HIGH;
                    userdata=LED0;pLed=lgThreadStart(Led1Hz,&userdata);break;
                }else{break;}           //動作状態のとき、何もせずに抜ける
            case 2:
                if(ledRun == LG_HIGH){  //動作状態のとき、Lチカスレッドストップ
                    ledRun=LG_LOW;
                    lgThreadStop(pLed);lgGpioWrite(g_hnd,LED0,LG_LOW);break;
                }else{break;}           //停止状態のとき、何もせずに抜ける
            case 3:
                if(orgelRun == LG_LOW){ //停止状態のとき、オルゴールスレッドスタート
                    orgelRun=LG_HIGH;
                    pOrgel=lgThreadStart(Orgel,&userdata);break;
                }else{break;}           //動作状態のとき、何もせずに抜ける
            case 4:
                if(orgelRun == LG_HIGH){ //動作状態のとき、オルゴールスレッドストップ
                    orgelRun=LG_LOW;
